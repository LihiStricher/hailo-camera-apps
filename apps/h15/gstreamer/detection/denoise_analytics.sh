...
/home/root/apps/clean_symlinks_config_isp.sh --mode "$mode" --tuning "$tuning_extension"
if [ $? -ne 0 ]; then
    echo "Failed to clean symlinks and copy ISP configuration files."
    exit 1
fi

echo "Running $network_name with SENSOR=${sensor:-default}, MODE=${mode:-default}, TUNING_EXTENSION=${tuning_extension:-none}"
echo ${PIPELINE}
