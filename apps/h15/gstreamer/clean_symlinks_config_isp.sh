...
function show_usage() {
    ...
    echo "  --sensor, -s SENSOR         Set the sensor name (default: auto-detected)"
    echo "  --lens, -l LENS             Set the lens name (default: theia_sl410m)"
    echo "  --help, -h                  Show this help message"
    echo
    echo "Example:"
    echo "  $0 --mode night --tuning _low_light --resolution fhd"
}
...
function remove_symlink_and_copy_isp_configuration() {
    ...
    local SOURCE_3ACONFIG="/etc/imaging/cfg/$SENSOR/$LENS_NAME/$MAX_RESOLUTION/profiles/$MODE/tuning$TUNING_EXTENSION/3aconfig.json"
    local SOURCE_SENSOR_ENTRY="/etc/imaging/cfg/$SENSOR/$LENS_NAME/$MAX_RESOLUTION/profiles/$MODE/tuning$TUNING_EXTENSION/Sensor0_Entry.cfg"
