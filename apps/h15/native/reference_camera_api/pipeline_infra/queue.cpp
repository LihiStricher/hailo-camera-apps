...
#ifdef HAVE_PERFETTO
      ,
      m_counter_name("queue_" + m_name), m_counter_track(perfetto::DynamicString(m_counter_name), "queue level")
#endif
{
...
void Queue::push(BufferPtr buffer)
{
    ...
    m_queue.push(buffer);
    REFERENCE_CAMERA_TRACE_COUNTER(m_counter_track, m_queue.size());
