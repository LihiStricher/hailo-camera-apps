...
/**
 * @brief Class representing an asynchronous HailoRT stage for RGB input in a connected stage pipeline.
 * 
 * This class is responsible for managing the HailoRT inference model with RGB input format,
 * setting up buffer pools, and executing asynchronous inference jobs.
 */
class RGBHailortAsyncStage : public ConnectedStage
{
private:
    // pool members
    ...
    std::string m_input_layer_name; ///< Name of the input layer for RGB data.
...
public:
    /**
     * @brief Construct a new RGBHailortAsyncStage object.
     * 
     * @param name Name of the stage.
     * @param hef_path Path to the Hailo Execution File (HEF).
     * @param input_layer_name Name of the input layer for RGB data.
     ...
     * @param job_limit Limit on the number of active inference jobs.
     * @param scheduler_threshold Threshold for the scheduler.
     * @param dynamic_threshold Whether to use dynamic thresholding.
     * @param scheduler_timeout Timeout for the scheduler.
     * @param print_fps Whether to print frames per second information.
     * @param pool_mode Pool mode for the buffer pool.
     */
    RGBHailortAsyncStage(std::string name, std::string hef_path, std::string input_layer_name, size_t queue_size, int output_pool_size, 
                         std::string group_id, int batch_size, size_t job_limit,
                         int scheduler_threshold = 4, bool dynamic_threshold = false, 
        ...
        : ConnectedStage(name, queue_size, false, print_fps), m_output_pool_size(output_pool_size), 
          m_hef_path(hef_path), m_group_id(group_id), m_input_layer_name(input_layer_name),
          m_batch_size(batch_size), m_scheduler_threshold(scheduler_threshold), 
          m_dynamic_threshold(dynamic_threshold), m_scheduler_timeout(scheduler_timeout), 
          m_jobs_limit(job_limit), m_pool_mode(pool_mode)
    {
    ...
    /**
     * @brief Process the data in the buffer with RGB input format.
     * 
     * @param data Buffer containing the RGB data to be processed.
    ...
    AppStatus process(BufferPtr data) override
    {
        ...
        // Set the input buffer with RGB data
        REFERENCE_CAMERA_LOG_DEBUG("[{}] Setting RGB input buffer for inference", m_stage_name);
        auto status = m_bindings.input(m_input_layer_name)->set_buffer(
            hailort::MemoryView(data->get_buffer()->get_plane_ptr(0), 
                               data->get_buffer()->get_plane_size(0) + data->get_buffer()->get_plane_size(1)));
        if (HAILO_SUCCESS != status)
        {
            REFERENCE_CAMERA_LOG_ERROR("[{}] Failed to set RGB input buffer", m_stage_name);
            return AppStatus::HAILORT_ERROR;
        }
