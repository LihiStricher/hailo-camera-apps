...
#define YOLO_HEF_FILE "/home/root/apps/case_studies/dynamic_privacy_mask/resources/yolov5l_seg_1class.hef"
                                  ...
                                  const std::string &hef_file_path, float nms_score_threshold)
{
    ...
    std::shared_ptr<HailortAsyncStage> segmentation_stage = HailortAsyncStageBuild::create()
...
void create_encoder_and_udp(const std::string &id, std::shared_ptr<AppResources> app_resources)
{
    ...
    std::shared_ptr<EncoderStage> encoder_stage = EncoderStageBuild::create()
    ...
    std::shared_ptr<UdpStage> udp_stage = UdpStageBuild::create()
                                     ...
                                     float nms_score_threshold)
{
    ...
    application_analytics_config_t analytics_config = {
        .instance_segmentation_analytics_config = {{ANALYTICS_DATA_ID,
                                                    {.analytics_data_id = ANALYTICS_DATA_ID,
                                                     .scaling_mode = "normal",
                                                     .width = AI_WIDTH,
                                                     .height = AI_HEIGHT,
                                                     .width_ratio = 1,
    ...
    auto &analytics_db = app_resources->media_library->get_analytics_db();
    analytics_db.initialize(analytics_config);
