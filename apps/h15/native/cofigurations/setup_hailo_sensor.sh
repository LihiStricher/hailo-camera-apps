...
if [ ! -d "/etc/imaging/cfg/$SENSOR_NAME" ]; then
    echo "Error: No configuration directory found for sensor $SENSOR_NAME in /etc/imaging/cfg"
    exit 1
fi

# Check if the lens directory exists
if [ ! -d "/etc/imaging/cfg/$SENSOR_NAME/$LENS_NAME" ]; then
    echo "Error: No configuration directory found for lens $LENS_NAME with sensor $SENSOR_NAME"
    exit 1
fi

# Set configuration directory
CONFIG_DIR="/etc/imaging/cfg/$SENSOR_NAME/$LENS_NAME"
