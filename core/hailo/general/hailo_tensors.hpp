...
public:
    ...
    HailoTensor(uint8_t *data, const hailo_tensor_metadata_t &tensor_meta_info) :
        m_data(data), 
        m_name(tensor_meta_info.name),
        m_width(tensor_meta_info.shape.width),
        m_height(tensor_meta_info.shape.height),
        m_features(tensor_meta_info.shape.features),
        m_qp_zp(tensor_meta_info.quant_info.qp_zp),
        m_qp_scale(tensor_meta_info.quant_info.qp_scale),
        m_is_uint16(tensor_meta_info.format.type == HailoTensorFormatType::HAILO_FORMAT_TYPE_UINT16), // Check if the format is uint16
    ...
    HailoTensor(uint8_t *data, const hailo_vstream_info_t &vstream_info) :
        m_data(data), 
        m_name(vstream_info.name),
        m_width(vstream_info.shape.width),
        m_height(vstream_info.shape.height),
        m_features(vstream_info.shape.features),
        m_qp_zp(vstream_info.quant_info.qp_zp),
        m_qp_scale(vstream_info.quant_info.qp_scale)
    {
    ...
    const float32_t qp_zp() { return m_qp_zp; }
    const float32_t qp_scale() { return m_qp_scale; }
