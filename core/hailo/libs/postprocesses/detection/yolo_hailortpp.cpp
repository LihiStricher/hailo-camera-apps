...
void yolov8n_personface(HailoROIPtr roi)
{
    ...
    for (auto &detection : detections)
    {
        ...
        float scale = 1.4f;
        float new_w = bbox.width() * scale;
        float new_h = bbox.height() * scale;
        float new_xmin = std::max(cx - new_w / 2.0f, 0.0f);
        float new_ymin = std::max(cy - new_h / 2.0f, 0.0f);

        HailoBBox new_bbox(new_xmin * roi_bbox.width() + roi_bbox.xmin(),
